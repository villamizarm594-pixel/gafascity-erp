...
const seed = {
  ...
  settings: { businessName:'GafasCity ERP', subtitle:'Gestion optica interna', logo:'', versionTitle:'Version 6', versionDescription:'Filtros avanzados, busquedas por fecha y exportacion filtrada', exchangeRate:0, exchangeRateDate:today() }
};
...
function Orders({orders,labs,setList}){
  ...
  <Card title="Filtros de ordenes" wide><div className="formGrid"><Input v={filters.q} p="Buscar cliente, telefono, formula, orden" on={v=>setFilters({...filters,q:v})}/><Input v={filters.from} p="Desde" type="date" on={v=>setFilters({...filters,from:v})}/><Input v={filters.to} p="Hasta" type="date" on={v=>setFilters({...filters,to:v})}/><Select v={filters.lab} on={v=>setFilters({...filters,lab:v})} opts={['Todos',...labs.map(l=>l.name)].map(x=>[x,x])}/><Select v={filters.status} on={v=>setFilters({...filters,status:v})} opts={['Todos','En la tienda','Enviado','Proceso','Entregado'].map(x=>[x,x])}/><Select v={filters.labPayment} on={v=>setFilters({...filters,labPayment:v})} opts={['Todos','No pago','Pago'].map(x=>[x,x])}/><Select v={filters.notified} on={v=>setFilters({...filters,notified:v})} opts={['Todos','No','Si'].map(x=>[x,x])}/><div className="totalBox">Ordenes:<b>{filteredOrders.length}</b></div><div className="totalBox">Resta total:<b>{money(pendingBalance)}</b></div><div className="totalBox">Deuda lab:<b>{money(labDebt)}</b></div><button className="secondary" onClick={()=>downloadCSV('ordenes-filtradas', filteredOrders)}>Exportar filtrado</button></div></Card>
...
function Config({store,setStore}){
  ...
  return <div className="stack">
    ...
    <Card title="Vista previa" wide>
      ...
      <div className="statusBox"><b>{settings.versionTitle || 'Version 6'}</b><span>{settings.versionDescription || 'Filtros avanzados y exportacion filtrada.'}</span></div>
    </Card>
  </div>
}
