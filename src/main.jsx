...
const seed = {
  ...
  settings: { businessName:'GafasCity ERP', subtitle:'Gestion optica interna', logo:'', versionTitle:'Producción Final estable', versionDescription:'Inventario físico y catálogo de cristales separados', commissionsEnabled:false, exchangeRate:0, exchangeRateDate:today() }
};
...
function SmartFinder({items,value,onSelect,label,mode='all',placeholder}){
  ...
  const list=items.filter(x=>!term||getText(x).toLowerCase().includes(term.toLowerCase())).slice(0,10);
  return <div className="smartFinder"><label className="field compactField"><FieldLabel p={label}/><input value={open?term:(mode==='code'?(selected?.code||''):mode==='description'?(selected?.description||''):(selected?[selected.crystal,selected.treatment].filter(Boolean).join(' · '):''))} placeholder={placeholder||label} onFocus={()=>{setOpen(true);setTerm('')}} onChange={e=>{setTerm(e.target.value);setOpen(true)}}/></label>{open&&<div className="finderResults">{list.length?list.map(x=><button type="button" className="finderOption" key={x.id} onClick={()=>{onSelect(x);setOpen(false);setTerm('')}}><b>{x.code||x.crystal}</b><span>{x.description||[x.treatment,x.colors,x.range,money(x.price)].filter(Boolean).join(' · ')}</span></button>):<div className="noResults">Sin coincidencias</div>}</div>}</div>}
...
function CrystalCatalog({catalog,setStore}){
...
function Config({store,setStore}){
  ...
  return <div className="stack">
    ...
    <Card title="Vista previa" wide>
      ...
      <div className="statusBox"><b>{settings.versionTitle || 'Producción Final estable'}</b><span>{settings.versionDescription || 'Inventario físico y catálogo de cristales separados.'}</span></div>
    </Card>
  </div>
}
