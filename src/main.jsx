...
const seed = {
  ...
  settings: { businessName:'GafasCity ERP', subtitle:'Gestion optica interna', logo:'', versionTitle:'Produccion Final 3.0', versionDescription:'Inventario físico y catálogo de cristales separados', commissionsEnabled:false, exchangeRate:0, exchangeRateDate:today() }
};
...
function Sales({store,setStore,currentUser,employeeMode=false}){
  ...
  const complete=()=>{if(!sale.customerName)return alert('Nombre del cliente obligatorio');if(!lineRows.length||lineRows.some(l=>!l.product))return alert('Selecciona todos los códigos de lente');if(lineRows.some(l=>Number(l.qty)<=0||Number(l.product.stock)<Number(l.qty)))return alert('Cantidad inválida o stock insuficiente');const newSale={id:uid(),date:sale.date,registeredDate:today(),customerName:sale.customerName,items:lineRows.map(l=>({productId:l.product.id,code:l.product.code,description:l.product.description,category:l.product.category,qty:Number(l.qty),unitPrice:l.unitPrice,total:l.lineTotal})),productCodeName:lineRows.map(l=>`${l.product.code} - ${l.product.description}`).join(' + '),qty:lineRows.reduce((a,l)=>a+Number(l.qty),0),payment:sale.payment,reference:sale.reference,subtotal,discountType:sale.discountType,discountValue:rawDiscount,discountAmount,total,totalBs,paidAmount:paid,paidAmountBs:paid*rate,balance,balanceBs:balance*rate,paymentStatus:balance<=0?'Pagado':(paid>0?'Abonado':'Pendiente'),exchangeRate:rate,sellerName,sellerEmail:currentUser,delayed:sale.date!==today(),omitCommission:false,cancelled:false};setStore(prev=>({...prev,sales:[...(prev.sales||[]),newSale],products:(prev.products||[]).map(p=>{const sold=lineRows.filter(l=>l.product.id===p.id).reduce((a,l)=>a+Number(l.qty),0);return sold?{...p,stock:Number(p.stock)-sold}:p})}),'Venta guardada');setSale({date:today(),customerName:'',payment:'Efectivo',initialPayment:'',reference:'',discountType:'Sin descuento',discountValue:'',lines:[makeLine()]});};
...
function Config({store,setStore}){
  ...
  return <div className="stack">
    ...
    <Card title="Vista previa" wide>
      ...
      <div className="statusBox"><b>{settings.versionTitle || 'Producción Final 3.4'}</b><span>{settings.versionDescription || 'Inventario físico y catálogo de cristales separados.'}</span></div>
    </Card>
  </div>
}
