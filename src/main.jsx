...
function EmployeeSales({store,setStore,currentUser}){
  ...
  return <div className="stack">
    ...
    <Card title="Registrar venta" wide><div className="sellerBanner">Venta registrada por: <b>{sellerName}</b></div><div className="formGrid"><Input v={sale.date} p="Fecha" type="date" on={v=>setSale({...sale,date:v})}/><Input v={sale.customerName} p="Nombre y apellido del cliente" on={v=>setSale({...sale,customerName:v})}/><Select p="Categoría" v={sale.category} on={v=>{const filtered=(store.products||[]).filter(p=>p.category===v);setSale({...sale,category:v,productId:filtered[0]?.id||''})}} opts={saleCategoryOptions.map(x=>[x,x])}/><Select p="Código de lente" v={sale.productId} on={v=>setSale({...sale,productId:v})} opts={(categoryProducts.length?categoryProducts:store.products).map(p=>[p.id,`${p.code} - ${p.description} (${p.stock})`])}/><div className="totalBox">Descripcion:<b>{product?.description||'-'}</b></div><Input v={sale.qty} p="Cantidad" type="number" on={v=>setSale({...sale,qty:v})}/><Select p="Método de pago" v={sale.payment} on={v=>setSale({...sale,payment:v})} opts={paymentOptions.map(x=>[x,x])}/><Input v={sale.initialPayment} p="Abono inicial" type="number" on={v=>setSale({...sale,initialPayment:v})}/><div className="totalBox">Total USD:<b>{money(total)}</b></div><div className="totalBox">Total Bs:<b>{rate?bs(totalBs):'Configurar tasa'}</b></div><Select p="Garantía" v={sale.warranty} on={v=>setSale({...sale,warranty:v})} opts={[["No","Sin garantia"],["Si","Con garantia"]]}/><button onClick={complete}><Save size={16}/>Registrar venta</button></div></Card>
...
function EmployeeInventoryAdd({store,setStore}){
  ...
  const filtered=products.filter(p=>containsText(p,q));
...
function PaymentsModule({store,setStore,currentUser}){
  ...
  return <div className="stack"><Guide title="Abonos / Cuentas por cobrar" items={["Busca una venta pendiente por cliente o código.","Registra cada abono con monto, método y referencia.","El saldo se actualiza automáticamente."]}/><Card title="Registrar abono" wide><div className="formGrid"><Input v={q} p="Buscar cliente o venta" on={setQ}/><Select p="Venta pendiente" v={form.saleId||selected?.id||''} on={v=>setForm({...form,saleId:v})} opts={openSales.map(s=>[s.id,`${s.customerName} - ${s.productCodeName} - Resta ${money(saleBalance(s))}`])}/><Input v={form.amount} p="Monto abonado" type="number" on={v=>setForm({...form,amount:v})}/><Select p="Método de pago" v={form.method} on={v=>setForm({...form,method:v})} opts={paymentOptions.map(x=>[x,x])}/><Input v={form.reference} p="Referencia" on={v=>setForm({...form,reference:v})}/><Input v={form.notes} p="Observaciones" on={v=>setForm({...form,notes:v})}/><button onClick={register}>Registrar abono</button></div></Card><Card title="Ventas pendientes" wide><Table rows={openSales} columns={[["customerName","Cliente"],["productCodeName","Código de lente"],["total","Total",money],["paidAmount","Abonado",v=>money(v||0)],["balance","Resta",v=>money(v||0)],["sellerName","Vendedor",v=>v||'-']]}/></Card><Card title="Historial de abonos" wide><Table rows={history} columns={[["date","Fecha"],["registeredBy","Registrado por"],["amount","Monto",money],["method","Método"],["reference","Referencia"],["notes","Observaciones"]]}/></Card></div>;
}

function CommissionsModule({sales}){
  const [from,setFrom]=useState(today()); const [to,setTo]=useState(today());
  const rows=(sales||[]).filter(s=>!s.cancelled && inDateRange(s.date,from,to));
  const sellers=['Rhoy','Ely','Anyi','Yngrid'];
  const summary=sellers.map(name=>{const ss=rows.filter(s=>s.sellerName===name && !s.omitCommission); const base=ss.reduce((a,s)=>a+salePaid(s),0); return {id:name,name, sales:ss.length, base, commission:base*COMMISSION_RATE};});
  return <div className="stack"><Card title="Comisiones" wide><div className="formGrid"><Input v={from} p="Desde" type="date" on={setFrom}/><Input v={to} p="Hasta" type="date" on={setTo}/><div className="totalBox">Tasa comisión:<b>{(COMMISSION_RATE*100).toFixed(0)}%</b></div></div></Card><Card title="Resumen por empleado" wide><Table rows={summary} columns={[["name","Empleado"],["sales","Ventas"],["base","Base cobrada",money],["commission","Comisión",money]]}/></Card><Card title="Detalle de ventas" wide><Table rows={rows} columns={[["date","Fecha venta"],["registeredDate","Fecha registro"],["sellerName","Vendedor"],["customerName","Cliente"],["productCodeName","Código de lente"],["paidAmount","Cobrado",v=>money(v||0)],["delayed","Retraso",v=>v?'Sí':'No'],["omitCommission","Comisión",v=>v?'Omitida':'Activa']]}/></Card></div>;
}

function DelayedSalesModule({sales,setStore}){
  const rows=(sales||[]).filter(s=>s.delayed && !s.cancelled);
  const toggle=(id)=>setStore(prev=>({...prev,sales:(prev.sales||[]).map(s=>s.id===id?{...s,omitCommission:!s.omitCommission}:s)}),'Venta actualizada');
  return <div className="stack"><Guide title="Ventas con retraso" items={["Aparecen ventas cuya fecha de venta no coincide con la fecha real de registro.","El administrador decide si esa venta cuenta o no para comisión."]}/><Card title="Omitir / aprobar comisión" wide><Table rows={rows} columns={[["date","Fecha venta"],["registeredDate","Fecha registro"],["sellerName","Vendedor"],["customerName","Cliente"],["total","Total",money],["omitCommission","Estado",v=>v?'Comisión omitida':'Comisión activa'],["actions","Acción",(_,r)=><button className="mini warn" onClick={()=>toggle(r.id)}>{r.omitCommission?'Aprobar comisión':'Omitir comisión'}</button>]]}/></Card></div>;
}

function ClinicalModule({store,setStore,currentUser}){
  const labs=Array.isArray(store.laboratories)?store.laboratories:[];
  const blank={number:`HC-${String(((store.orders||[]).length||0)+1).padStart(4,'0')}`,date:today(),customer:'',phone:'',idCard:'',age:'',clinicalHistory:'',reason:'',lab:labs[0]?.name||'Novak',lens:'',treatment:'',odEsf:'',odCil:'',odEje:'',odAdd:'',oiEsf:'',oiCil:'',oiEje:'',oiAdd:'',dp:'',height:'',frame:'',status:'En la tienda',notes:''};
  const [form,setForm]=useState(blank);
  const [q,setQ]=useState('');
  const save=()=>{if(!form.customer||!form.phone)return alert('Cliente y teléfono son obligatorios'); const prescription=`OD: Esf ${form.odEsf||'-'} | Cil ${form.odCil||'-'} | Eje ${form.odEje||'-'} | Add ${form.odAdd||'-'} / OI: Esf ${form.oiEsf||'-'} | Cil ${form.oiCil||'-'} | Eje ${form.oiEje||'-'} | Add ${form.oiAdd||'-'} / DP: ${form.dp||'-'} / Altura: ${form.height||'-'}`; const order={...form,id:uid(),orderNumber:form.number,lab:form.lab,prescription,total:0,deposit:0,balance:0,labPayment:'No pago',notifiedClient:'No',createdBy:getStaffName(currentUser),createdEmail:currentUser,clinical:true}; setStore(prev=>({...prev,orders:[...(prev.orders||[]),order]}),'Fórmula guardada'); setForm(blank);};
  const rows=(store.orders||[]).filter(o=>(o.clinical||o.prescription) && containsText(o,q)).slice().reverse();
  return <div className="stack"><Guide title="Fórmulas / Historial clínico" items={["Módulo para registrar fórmulas, historial clínico y órdenes ópticas.","Completa los datos del paciente y la prescripción OD/OI.","La información queda disponible para seguimiento de trabajos."]}/><Card title="Nueva fórmula / historia clínica" wide><div className="formGrid"><Input v={form.number} p="Código" on={v=>setForm({...form,number:v})}/><Input v={form.date} p="Fecha" type="date" on={v=>setForm({...form,date:v})}/><Input v={form.customer} p="Paciente" on={v=>setForm({...form,customer:v})}/><Input v={form.phone} p="Teléfono" on={v=>setForm({...form,phone:v})}/><Input v={form.idCard} p="Cédula" on={v=>setForm({...form,idCard:v})}/><Input v={form.age} p="Edad" type="number" on={v=>setForm({...form,age:v})}/><Input v={form.reason} p="Motivo de consulta" on={v=>setForm({...form,reason:v})}/><Input v={form.clinicalHistory} p="Historial clínico" on={v=>setForm({...form,clinicalHistory:v})}/><Select p="Laboratorio" v={form.lab} on={v=>setForm({...form,lab:v})} opts={(labs.length?labs:[{name:'Novak'}]).map(l=>[l.name,l.name])}/><Input v={form.lens} p="Cristal" on={v=>setForm({...form,lens:v})}/><Input v={form.treatment} p="Tratamiento" on={v=>setForm({...form,treatment:v})}/><div className="rxBox"><div className="rxTitle">Prescripción óptica</div><div className="rxGrid rxHeader"><span></span><span>Esf</span><span>Cil</span><span>Eje</span><span>Add</span></div><div className="rxGrid"><b>OD</b><Input v={form.odEsf} p="OD Esf" on={v=>setForm({...form,odEsf:v})}/><Input v={form.odCil} p="OD Cil" on={v=>setForm({...form,odCil:v})}/><Input v={form.odEje} p="OD Eje" on={v=>setForm({...form,odEje:v})}/><Input v={form.odAdd} p="OD Add" on={v=>setForm({...form,odAdd:v})}/></div><div className="rxGrid"><b>OI</b><Input v={form.oiEsf} p="OI Esf" on={v=>setForm({...form,oiEsf:v})}/><Input v={form.oiCil} p="OI Cil" on={v=>setForm({...form,oiCil:v})}/><Input v={form.oiEje} p="OI Eje" on={v=>setForm({...form,oiEje:v})}/><Input v={form.oiAdd} p="OI Add" on={v=>setForm({...form,oiAdd:v})}/></div><div className="rxGrid rxTwo"><Input v={form.dp} p="DP" on={v=>setForm({...form,dp:v})}/><Input v={form.height} p="Altura" on={v=>setForm({...form,height:v})}/></div></div><Input v={form.frame} p="Montura" on={v=>setForm({...form,frame:v})}/><Select p="Estatus" v={form.status} on={v=>setForm({...form,status:v})} opts={['En la tienda','Enviado','Proceso','Entregado'].map(x=>[x,x])}/><Input v={form.notes} p="Observaciones" on={v=>setForm({...form,notes:v})}/><button onClick={save}>Guardar fórmula</button></div></Card><Card title="Historial clínico / fórmulas" wide><div className="search"><Search size={16}/><input placeholder="Buscar paciente, teléfono u orden" value={q} onChange={e=>setQ(e.target.value)}/></div><Table rows={rows} columns={[["number","Código"],["customer","Paciente"],["phone","Teléfono"],["lab","Laboratorio"],["status","Estatus"],["prescription","Fórmula"],["createdBy","Registrado por"]]}/></Card></div>;
}
