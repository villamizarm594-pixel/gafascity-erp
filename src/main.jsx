...
const seed = {
  ...
  settings: { businessName:'GafasCity ERP', subtitle:'Gestion optica interna', logo:'', versionTitle:'Producción Final abonos con fecha', versionDescription:'Inventario físico y catálogo de cristales separados', commissionsEnabled:false, exchangeRate:0, exchangeRateDate:today() }
};
...
function Sales({store,setStore,currentUser,employeeMode=false}){
  ...
  const makeLine=()=>({id:uid(),category:'Montura',productId:'',qty:1,agreedPrice:''});
  ...
  const lineRows=sale.lines.map(l=>{const p=products.find(x=>x.id===l.productId);const unitPrice=l.category==='Montura'?Number(l.agreedPrice||0):Number(p?.price||0);return {...l,product:p,description:p?.description||'',unitPrice,lineTotal:unitPrice*Number(l.qty||0)}});
...
function Orders({store,setStore,currentUser,isAdmin}){
 ...
 const makeWork=()=>({id:uid(),open:true,prescriptionIndex:0,frameId:'',frameQty:1,frameAgreedPrice:'',frameWarranty:'No',crystalId:'',crystalQty:1,crystalWarranty:'No',additionals:[]});
...
function Config({store,setStore}){
  ...
  return <div className="stack">
    ...
    <Card title="Vista previa" wide>
      ...
      <div className="statusBox"><b>{settings.versionTitle || 'Producción Final abonos con fecha'}</b><span>{settings.versionDescription || 'Inventario físico y catálogo de cristales separados.'}</span></div>
    </Card>
  </div>
}
