...
const seed = {
  ...
  settings: { businessName:'GafasCity ERP', subtitle:'Gestion optica interna', logo:'', versionTitle:'Producción Final precios empleados', versionDescription:'Inventario físico y catálogo de cristales separados', commissionsEnabled:false, exchangeRate:0, exchangeRateDate:today() }
};
...
function App(){
  ...
  const adminNav = [
    ['dashboard','Inicio',LayoutDashboard], ['sales','Ventas',ShoppingCart], ['payments','Abonos / Cuentas',Wallet], ['clinical','Fórmulas / Historial',ClipboardList], ['orders','Ordenes / formulas',ClipboardList], ['labs','Laboratorios',Microscope], ['inventory','Inventario',Package], ['crystals','Catálogo cristales',Search], ['customers','Clientes',Users], ['cash','Caja diaria',Wallet], ['expenses','Gastos',Receipt], ['commissions','Comisiones',BarChart3], ['delayed','Límite de entrega',Receipt], ['reports','Reportes',BarChart3], ['config','Configuracion',Settings]
  ];
  const employeeNav = [
    ['sales','Registrar venta',ShoppingCart], ['inventoryAdd','Agregar inventario',Package], ['crystals','Catálogo cristales',Search], ['orders','Órdenes y fórmulas',ClipboardList], ['payments','Abonos',Wallet], ['labs','Laboratorios',Microscope], ['tracking','Seguimiento de trabajos',ClipboardList]
  ];
  const formulaNav = [
...
function SmartFinder({items,value,onSelect,onClear,label,mode='all',placeholder,showMeta=false}){
  ...
  const getText=x=>mode==='code'?String(x.code||''):mode==='description'?String(x.description||''):[x.code,x.description,x.crystal,x.treatment,x.colors,x.range].filter(Boolean).join(' ');
  const list=items.filter(x=>!term||getText(x).toLowerCase().includes(term.toLowerCase())).slice(0,8);
...
function EmployeeInventoryAdd({store,setStore,currentUser}){
 const blank={code:'',date:today(),category:'Montura',description:'',qty:'',cost:'',price:'',stock:'',minStock:'1'};
 const [form,setForm]=useState(blank),[selectedId,setSelectedId]=useState(null),[filters,setFilters]=useState({code:'',description:''}),[priceEdit,setPriceEdit]=useState(null),[newPrice,setNewPrice]=useState('');
 ...
 const save=()=>{if(!form.code||!form.description)return alert('Código y descripción son obligatorios');if(incoming<=0)return alert('Indica la cantidad a ingresar');if(selected){setStore(prev=>({...prev,products:(prev.products||[]).map(p=>p.id===selected.id?{...p,stock:newStock,qty:Number(p.qty||0)+incoming}:p)}),'Stock actualizado');}else{const duplicate=products.find(p=>String(p.code).toLowerCase()===String(form.code).toLowerCase());if(duplicate)return alert(`El código ya pertenece a ${duplicate.description}. Selecciónalo en el buscador.`);setStore(prev=>({...prev,products:[...(prev.products||[]),{...form,id:uid(),qty:incoming,stock:incoming,minStock:Number(form.minStock||1),cost:Number(form.cost||0),price:Number(form.price||0)}]}),'Producto agregado');}setForm(blank);setSelectedId(null)};
 const openPrice=p=>{setPriceEdit(p);setNewPrice(String(Number(p.price||0)||''))};
 const savePrice=()=>{const value=Number(newPrice);if(!Number.isFinite(value)||value<=0)return alert('El precio debe ser mayor que cero');const previous=Number(priceEdit.price||0);setStore(prev=>({...prev,products:(prev.products||[]).map(p=>p.id===priceEdit.id?{...p,price:value,pricePending:false,lastPriceChange:{previous,new:value,by:getStaffName(currentUser),email:currentUser,date:today()}}:p)}),'Precio actualizado');setPriceEdit(null);setNewPrice('')};
 ...
 return <div className="stack"><Guide title="Agregar inventario" items={["Busca por código o descripción para sumar existencias sin duplicar productos.","También puedes asignar o corregir únicamente el precio de venta desde la tabla."]}/><Card title={selected?'Sumar existencias':'Agregar producto'} wide><div className="inventorySearchGrid"><SmartFinder items={products} value={selectedId} onSelect={choose} label="Buscar por código" mode="code"/><SmartFinder items={products} value={selectedId} onSelect={choose} label="Buscar por descripción" mode="description"/></div><div className="formGrid"><Input v={form.code} p="Código" on={v=>{setSelectedId(null);setForm({...form,code:v})}}/><Input v={form.date} p="Fecha" type="date" on={v=>setForm({...form,date:v})}/><Select p="Categoría" v={form.category} on={v=>setForm({...form,category:v})} opts={saleCategoryOptions.filter(x=>x!=='Cristales').map(x=>[x,x])}/><Input v={form.description} p="Descripción" on={v=>{setSelectedId(null);setForm({...form,description:v})}}/><Input v={form.qty} p={selected?'Cantidad a ingresar':'Cantidad inicial'} type="number" on={v=>setForm({...form,qty:v})}/><div className="totalBox"><span>Stock actual</span><b>{selected?current:0}</b></div><div className="totalBox strong"><span>Nuevo stock</span><b>{selected?newStock:incoming}</b></div><Input v={form.minStock} p="Nivel mínimo de alerta" type="number" on={v=>setForm({...form,minStock:v})}/>{!selected&&<><Input v={form.cost} p="Costo" type="number" on={v=>setForm({...form,cost:v})}/><Input v={form.price} p="Precio" type="number" on={v=>setForm({...form,price:v})}/></>}<button onClick={save}><Save size={16}/>{selected?'Sumar al inventario':'Agregar producto'}</button></div></Card><Card title="Consultar inventario" wide><div className="formGrid"><Input v={filters.code} p="Filtrar código" on={v=>setFilters({...filters,code:v})}/><Input v={filters.description} p="Filtrar descripción" on={v=>setFilters({...filters,description:v})}/></div>{priceEdit&&<div className="priceEditor"><div><b>{priceEdit.code}</b><span>{priceEdit.description}</span><small>Precio actual: {money(priceEdit.price)}</small></div><Input v={newPrice} p="Nuevo precio USD" type="number" on={setNewPrice}/><button onClick={savePrice}>Guardar precio</button><button className="secondary" onClick={()=>setPriceEdit(null)}>Cancelar</button></div>}<div className="innerScroll"><Table rows={filtered} columns={[["code","Código"],["category","Categoría"],["description","Descripción"],["stock","Stock actual"],["minStock","Nivel mínimo"],["price","Precio",money],["actions","Acciones",(_,r)=><button className="mini secondary" onClick={()=>openPrice(r)}>Editar precio</button>]]}/></div></Card></div>;
}
...
function CommissionsModule({sales,orders,enabled}){
  const [from,setFrom]=useState(today()),[to,setTo]=useState(today());
  if(!enabled)return <div className="stack"><Card title="Comisiones desactivadas" wide><div className="disabledFeature"><b>Las comisiones no se están calculando actualmente.</b><span>El administrador puede activarlas en Configuración cuando decida comenzar a utilizarlas.</span></div></Card></div>;
...
function Sales({store,setStore,currentUser,employeeMode=false}){
  ...
  const [sale,setSale]=useState({date:today(),customerName:'',payment:'Efectivo',initialPayment:'',reference:'',discountType:'Sin descuento',discountValue:'',lines:[makeLine()]});
...
function Orders({store,setStore,currentUser,isAdmin}){
 ...
 const totals=form.works.map(w=>{const frame=products.find(p=>p.id===w.frameId),crystal=catalog.find(c=>c.id===w.crystalId);const ft=Number(frame?.price||0)*Number(w.frameQty||0),ct=Number(crystal?.price||0)*Number(w.crystalQty||0),at=(w.additionals||[]).reduce((a,x)=>a+Number(x.price||0),0);return {...w,frame,crystal,frameTotal:ft,crystalTotal:ct,additionalTotal:at,total:ft+ct+at}});
 const total=totals.reduce((a,w)=>a+w.total,0),deposit=Number(form.deposit||0),balance=Math.max(0,total-deposit);
 const save=()=>{if(!form.customer||!form.idCard)return alert('Nombre y cédula son obligatorios');if(!prescriptions.length)return alert('Selecciona una fórmula de Yngrid o agrega una fórmula externa');if(totals.some(w=>w.frame&&Number(w.frame.price||0)<=0))return alert('Una montura no tiene precio');if(totals.some(w=>w.frame&&Number(w.frame.stock||0)<Number(w.frameQty||0)))return alert('Stock insuficiente en una montura');const order={...form,id:editing||uid(),total,totalBs:total*rate,deposit,depositBs:deposit*rate,balance,balanceBs:balance*rate,responsible,responsibleEmail:currentUser,clinical:false,works:totals.map(w=>({...w,frame:w.frame?{id:w.frame.id,code:w.frame.code,description:w.frame.description,price:w.frame.price}:null,crystal:w.crystal?{...w.crystal}:null})),formulaOrigin:form.externalFormula?'Externa':'GafasCity'};setStore(prev=>({...prev,orders:editing?(prev.orders||[]).map(o=>o.id===editing?order:o):[...(prev.orders||[]),order],products:(prev.products||[]).map(p=>{const used=totals.filter(w=>w.frame?.id===p.id).reduce((a,w)=>a+Number(w.frameQty||0),0);return used&&!editing?{...p,stock:Number(p.stock)-used}:p})}),'Orden guardada');setForm(blank());setEditing(null);setShowExternal(false)};
...
function Config({store,setStore}){
  ...
  return <div className="stack">
    ...
    <Card title="Vista previa" wide>
      ...
      <div className="statusBox"><b>{settings.versionTitle || 'Producción Final precios empleados'}</b><span>{settings.versionDescription || 'Inventario físico y catálogo de cristales separados.'}</span></div>
    </Card>
  </div>
}
